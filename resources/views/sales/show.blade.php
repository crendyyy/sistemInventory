<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Detail Penjualan: ') }} {{ $sale->invoice_no }}
        </h2>
    </x-slot>
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tfoot>
                                <tr class="font-bold text-gray-900 dark:text-white bg-gray-50 dark:bg-gray-700">
                                    ...
                                    <td class="px-4 py-3 text-right text-lg">Rp {{ number_format($sale->total, 0, ',', '.') }}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                    ...
                    @if($sale->cashTransactions->count() > 0)
                        <ul class="space-y-4">
                            @foreach($sale->cashTransactions as $cash)
        ...
        <div>
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <div class="space-y-4">
                        <div>
                            <span class="block text-sm text-gray-500 dark:text-gray-400">Customer</span>
                            <span class="block font-medium text-gray-900 dark:text-white">{{ $sale->customer->kode ?? '' }} - {{ $sale->customer->name ?? 'Tidak ada' }}</span>
                        </div>
                        <div>
                            <span class="block text-sm text-gray-500 dark:text-gray-400">Tanggal Transaksi</span>
                            <span class="block font-medium text-gray-900 dark:text-white">{{ \Carbon\Carbon::parse($sale->sale_date)->format('d F Y') }}</span>
                        </div>
                        <div>
